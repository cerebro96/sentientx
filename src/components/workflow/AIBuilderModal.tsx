...
interface AIBuilderModalProps {
  ...
  workflowId?: string; // Workflow ID to associate chat with specific workflow
  sessionId?: string; // Optional session ID to load existing conversation
}
...
*The AI has designed a complete workflow based on your requirements. Ready when you are!*`;
  ...
  const handleGenerateWorkflow = async () => {
    ...
    try {
      // Use the stored JSON configuration to generate the workflow
      console.log('Generating workflow with config:', workflowJsonConfig);
      
      // Process the JSON configuration to match expected workflow format
      const generatedWorkflow = {
        // name: workflowJsonConfig.id || workflowJsonConfig.id || "AI Generated Workflow",
        // description: workflowJsonConfig.name || workflowJsonConfig.name || "Generated by AI Builder",
        // agent_type: workflowJsonConfig.agent_type || workflowJsonConfig.agent_type || "multi_agent",
        // config: workflowJsonConfig, // Store the full config
        nodes: workflowJsonConfig.nodes || [],
        edges: workflowJsonConfig.edges || [],
        // agents: workflowJsonConfig.agents || workflowJsonConfig.agent_breakdown || []
      };
